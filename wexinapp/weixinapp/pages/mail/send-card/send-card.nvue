<template>
	<view>
		...
		<free-list-item :title="item.username" v-for="(item,index) in allList" :key="index" :cover="item.avatar"
			showRight :showRightIcon="false" @click="selectItem(item)">
			<view v-if="muliSelect" slot="right" style="width: 40rpx;height: 40rpx;"
				class="border rounded-circle flex align-center justify-center">
				<view v-if="item.checked" style="width: 30rpx;height: 30rpx;" class="main-bg-color rounded-circle"></view>
			</view>
		</free-list-item>
